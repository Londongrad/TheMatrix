...
public sealed class FakeRedisDatabaseState
{
    ...
    public void SeedSet(string key, params string[] values)
    {
        _sets[key] = new HashSet<string>(values, StringComparer.Ordinal);
    }

    public IReadOnlyCollection<string> GetSetMembers(string key)
    {
        return _sets.TryGetValue(key, out HashSet<string>? values)
            ? values.ToArray()
            : [];
    }
    ...
    {
        return _sets.TryGetValue(key, out HashSet<string>? values)
            ? values.Select(static value => (RedisValue)value).ToArray()
            : [];
    }

    internal bool SetAdd(string key, string value)
    {
    ...
    internal long SetRemoveMany(string key, IReadOnlyCollection<string> values)
    {
        if (!_sets.TryGetValue(key, out HashSet<string>? existing))
            return 0;
    ...
    internal bool LockTake(string key, string token)
    {
    ...
    internal bool LockRelease(string key, string token)
    {
        if (_locks.TryGetValue(key, out string? existingToken) &&
            string.Equals(existingToken, token, StringComparison.Ordinal))
        {
            _locks.Remove(key);
    ...
    public void SeedLock(string key, string token)
    {
        _locks[key] = token;
    }
    ...
    {
        if (!_sets.TryGetValue(key, out HashSet<string>? values))
        {
...
public static class RedisTestDoubles
{
    public static IConnectionMultiplexer CreateConnectionMultiplexer(FakeRedisDatabaseState state)
    {
        ...
        IConnectionMultiplexer multiplexer = DispatchProxy.Create<IConnectionMultiplexer, ConnectionMultiplexerProxy>();
        ConnectionMultiplexerProxy proxy = (ConnectionMultiplexerProxy)(object)multiplexer;
        proxy.State = state;
    ...
    private static IDatabase CreateDatabase(FakeRedisDatabaseState state)
    {
        ...
        DatabaseProxy proxy = (DatabaseProxy)(object)database;
        proxy.State = state;
        return database;
    }
    ...
    private class DatabaseProxy : DispatchProxy
    {
        ...
        protected override object? Invoke(MethodInfo? targetMethod, object?[]? args)
        {
            ...
            return targetMethod.Name switch
            {
                ...
                nameof(IDatabase.SetAddAsync) => Task.FromResult(State.SetAdd(GetKey(args[0]), GetValue(args[1]))),
                nameof(IDatabase.SetRemoveAsync) => HandleSetRemoveAsync(args),
                nameof(IDatabase.LockTakeAsync) => Task.FromResult(State.LockTake(GetKey(args[0]), GetValue(args[1]))),
                nameof(IDatabase.LockReleaseAsync) => Task.FromResult(State.LockRelease(GetKey(args[0]), GetValue(args[1]))),
                _ => GetDefaultReturnValue(targetMethod.ReturnType)
            };
        }
        ...
        private object HandleSetRemoveAsync(object?[] args)
        {
            ...
            {
                RedisValue value => Task.FromResult(State.SetRemove(key, value.ToString())),
                RedisValue[] values => Task.FromResult(State.SetRemoveMany(key, values.Select(static value => value.ToString()).ToArray())),
