...
internal static class OutboxTestSupport
{
    ...
    internal static T DeserializePayload<T>(OutboxMessage message)
        where T : notnull
    {
        T? payload = JsonSerializer.Deserialize<T>(message.PayloadJson, JsonOptions);
    ...
    internal static CityEnvironmentChangedDomainEvent CreateCityEnvironmentChangedDomainEvent(
        Guid? cityId = null)
    {
