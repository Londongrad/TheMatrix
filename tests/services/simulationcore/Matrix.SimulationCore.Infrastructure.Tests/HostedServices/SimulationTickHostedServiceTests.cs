...
namespace Matrix.SimulationCore.Infrastructure.Tests.HostedServices
{
    public sealed class SimulationTickHostedServiceTests
    {
        ...
        {
            SimulationTickHostedService service = CreateService(
            ...
            Assert.Contains(
                expectedSubstring: "PeriodMilliseconds must be > 0",
        ...
        public async Task StartAsync_WhenFixedStepSecondsIsNonPositive_ThrowsInvalidOperationException()
        {
            ...
            Assert.Contains(
                expectedSubstring: "FixedStepSeconds must be > 0",
                actualString: exception.Message);
        }
        ...
        public async Task StartAsync_WhenMaxStepsPerSimulationPerCycleIsNonPositive_ThrowsInvalidOperationException()
        {
            ...
            Assert.Contains(
                expectedSubstring: "MaxStepsPerSimulationPerCycle must be > 0",
                actualString: exception.Message);
        }
        ...
        public async Task StartAsync_WhenTickOccurs_SendsAdvanceRunningSimulationsCommand()
        {
            ...
            Assert.Contains(
                ...
                filter: x => x.LogLevel == LogLevel.Debug && x.Message.Contains("real delta"));
        }
        ...
        public async Task StartAsync_WhenMediatorThrows_LogsErrorAndStopsOnCancellation()
        {
            ...
            Assert.Single(mediator.Commands);
            TestLogEntry errorEntry = Assert.Single(
                collection: logger.Entries,
                predicate: x => x.LogLevel == LogLevel.Error);
            Assert.Contains(
                expectedSubstring: "SimulationCore tick loop iteration failed.",
                actualString: errorEntry.Message);
        ...
        public async Task StartAsync_WhenResultHasLaggingSimulations_LogsWarningWithCapAndStepSize()
        {
            ...
            TestLogEntry warningEntry = Assert.Single(
                collection: logger.Entries,
                predicate: x => x.LogLevel == LogLevel.Warning);

            Assert.Contains(
                expectedSubstring: "2 simulations",
                actualString: warningEntry.Message);
            Assert.Contains(
                expectedSubstring: "10 fixed steps",
                actualString: warningEntry.Message);
            Assert.Contains(
                expectedSubstring: "10",
                actualString: warningEntry.Message);
            Assert.Contains(
                expectedSubstring: "00:01:00",
                actualString: warningEntry.Message);
        }
