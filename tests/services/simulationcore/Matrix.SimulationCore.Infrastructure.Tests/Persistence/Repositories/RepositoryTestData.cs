...
{
    internal static readonly DateTimeOffset BaseUtc = new(2048, 2, 3, 4, 5, 6, TimeSpan.Zero);
        ...
        WeatherState? currentState = null)
    {
        ...
        SimTime startedAt = SimTime.FromUtc(BaseUtc.AddHours(1));
        SimTime expectedUntil = startedAt.Add(TimeSpan.FromHours(3));

        return CityWeather.Create(
            cityId: cityId,
            climateProfile: CreateClimateProfile(),
            currentState: currentState ?? CreateWeatherState(startedAt, expectedUntil),
    ...
    internal static City CreateCity(DateTimeOffset? createdAtUtc = null, string name = "Clock City")
    {
