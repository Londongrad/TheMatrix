...
public sealed class CityRepositoryTests
{
    ...
    public async Task ListAsync_WhenArchivedCitiesAreExcluded_ReturnsOnlyActiveCitiesOrderedByCreatedAt()
    {
        ...
        DateTimeOffset baseUtc = new(2048, 2, 3, 4, 5, 6, TimeSpan.Zero);

        City activeOlder = SimulationInfrastructureTestSupport.CreateCity(baseUtc, name: "Active Older");
        City archivedCity = SimulationInfrastructureTestSupport.CreateCity(baseUtc.AddMinutes(1), name: "Archived");
        archivedCity.Archive(baseUtc.AddHours(1));
        City activeNewer = SimulationInfrastructureTestSupport.CreateCity(baseUtc.AddMinutes(2), name: "Active Newer");

        await dbContext.Cities.AddRangeAsync(activeOlder, archivedCity, activeNewer);
        await dbContext.SaveChangesAsync();
        var repository = new CityRepository(dbContext);

        IReadOnlyList<City> result = await repository.ListAsync(includeArchived: false, CancellationToken.None);

        Assert.Equal([activeOlder.Id, activeNewer.Id], result.Select(static x => x.Id).ToArray());
    }
    ...
    public async Task ListProvisioningAsync_ReturnsProvisioningFailuresFirstThenNewestItems()
    {
        ...
        failedCity.TryFailPopulationBootstrap(
            failedCity.PopulationBootstrapOperationId,
            "timeout",
        ...
        City activeCity = SimulationInfrastructureTestSupport.CreateCity(baseUtc.AddMinutes(3), name: "Active");

        await dbContext.Cities.AddRangeAsync(failedCity, provisioningOlder, provisioningNewer, activeCity);
        await dbContext.SaveChangesAsync();
        var repository = new CityRepository(dbContext);
        ...
        Assert.Equal(
            [failedCity.Id, provisioningNewer.Id, provisioningOlder.Id],
            result.Select(static x => x.Id).ToArray());
    }
    ...
    public async Task ListRecoverableProvisioningAsync_ReturnsExpiredAndUnleasedCitiesInRecoveryOrder()
    {
        ...
        City expiredLeaseCity = SimulationInfrastructureTestSupport.CreateCity(
            ...
            requiresPopulationBootstrap: true,
            name: "Expired Lease");
        expiredLeaseCity.TryAcquireProvisioningLease(baseUtc.AddMinutes(1), TimeSpan.FromMinutes(5));
        ...
        City activeLeaseCity = SimulationInfrastructureTestSupport.CreateCity(
            ...
            requiresPopulationBootstrap: true,
            name: "Active Lease");
        activeLeaseCity.TryAcquireProvisioningLease(asOfUtc.AddMinutes(-1), TimeSpan.FromMinutes(10));

        await dbContext.Cities.AddRangeAsync(expiredLeaseCity, noLeaseCity, activeLeaseCity);
        await dbContext.SaveChangesAsync();
        var repository = new CityRepository(dbContext);
        ...
        Assert.Equal([noLeaseCity.Id, expiredLeaseCity.Id], result.Select(static x => x.Id).ToArray());
    }
    ...
    public async Task GetByProvisioningCorrelationIdAsync_WhenCityExists_ReturnsMatchingCity()
    {
        ...
        Assert.Equal(provisioningCorrelationId, result.ProvisioningCorrelationId);
    }
}
