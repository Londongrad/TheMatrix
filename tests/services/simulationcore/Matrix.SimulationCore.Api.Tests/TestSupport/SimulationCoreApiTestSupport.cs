...
{
    public static ClockDto CreateClockDto(Guid? simulationId = null, Guid? hostId = null)
    {
        return new ClockDto(
            ...
            SimTimeUtc: new DateTimeOffset(2048, 6, 1, 9, 30, 0, TimeSpan.Zero),
            TickId: 42,
            Speed: 2.0m,
    ...
    public static CreateCityRequest CreateCreateCityRequest()
    {
        return new CreateCityRequest(
            ...
            StartSimTimeUtc: new DateTimeOffset(2048, 6, 1, 0, 0, 0, TimeSpan.Zero),
    ...
    public static CityDto CreateCityDto(Guid? cityId = null, string name = "Mega City", string status = "Provisioning")
    {
        Guid effectiveCityId = cityId ?? Guid.Parse("71b8b30b-d43c-4111-b1da-02c4d258cb18");
        DateTimeOffset now = new(2048, 6, 1, 10, 0, 0, TimeSpan.Zero);
    ...
    public static CityGenerationCatalogDto CreateGenerationCatalogDto()
    {
        return new CityGenerationCatalogDto(
            CityNamePresets: ["Mega City", "Neo Harbor"],
            DistrictNamePresets: ["North", "South"],
            StreetNamePresets: ["Central Avenue", "Orbital Road"]);
    }
    ...
    public static SuggestedCityNamesDto CreateSuggestedCityNamesDto(string? seed = "seed-42")
    {
        return new SuggestedCityNamesDto(
            Seed: seed,
            Names: ["Mega City", "Neo Harbor", "Copper Hill"]);
    }
    ...
    public static RoadNodeDto CreateRoadNodeDto(Guid cityId, Guid districtId)
    {
        return new RoadNodeDto(
            ...
            PositionX: 13.0m,
            PositionY: 41.0m,
            CreatedAtUtc: new DateTimeOffset(2048, 6, 1, 8, 5, 0, TimeSpan.Zero));
    }

    public static ResidentialBuildingDto CreateResidentialBuildingDto(Guid cityId, Guid districtId, Guid accessRoadNodeId)
    {
        return new ResidentialBuildingDto(
            ...
            PositionX: 14.0m,
            PositionY: 39.5m,
            CreatedAtUtc: new DateTimeOffset(2048, 6, 1, 8, 10, 0, TimeSpan.Zero));
    }

    public static CityAnchorDto CreateCityAnchorDto(Guid cityId, Guid districtId, Guid accessRoadNodeId)
    {
        return new CityAnchorDto(
            ...
            PositionX: 15.0m,
            PositionY: 38.0m,
            CreatedAtUtc: new DateTimeOffset(2048, 6, 1, 8, 15, 0, TimeSpan.Zero));
    }

    public static RoadSegmentDto CreateRoadSegmentDto(Guid cityId, Guid districtId, Guid fromRoadNodeId, Guid toRoadNodeId)
    {
        return new RoadSegmentDto(
            ...
            Type: "LocalRoad",
            LengthMeters: 240m,
            CreatedAtUtc: new DateTimeOffset(2048, 6, 1, 8, 20, 0, TimeSpan.Zero));
    }

    public static CityMapTopologyDto CreateMapTopologyDto(Guid cityId)
    {
        DistrictDto district = CreateDistrictDto(cityId);
        RoadNodeDto roadNode = CreateRoadNodeDto(cityId, district.DistrictId);
        ResidentialBuildingDto building = CreateResidentialBuildingDto(cityId, district.DistrictId, roadNode.RoadNodeId);
        CityAnchorDto anchor = CreateCityAnchorDto(cityId, district.DistrictId, roadNode.RoadNodeId);
        RoadSegmentDto segment = CreateRoadSegmentDto(cityId, district.DistrictId, roadNode.RoadNodeId, roadNode.RoadNodeId);
    ...
    public static CityWeatherDto CreateWeatherDto(Guid cityId)
    {
        return new CityWeatherDto(
            ...
            PressureHpa: 1017m,
            StartedAtUtc: new DateTimeOffset(2048, 6, 1, 9, 0, 0, TimeSpan.Zero),
            ExpectedUntilUtc: new DateTimeOffset(2048, 6, 1, 15, 0, 0, TimeSpan.Zero),
            LastEvaluatedAtUtc: new DateTimeOffset(2048, 6, 1, 9, 30, 0, TimeSpan.Zero),
            LastTransitionAtUtc: new DateTimeOffset(2048, 6, 1, 8, 45, 0, TimeSpan.Zero),
            ActiveOverride: null);
    }

    public static ResolveCityRouteRequest CreateResolveCityRouteRequest(Guid fromId, Guid toId)
    {
        return new ResolveCityRouteRequest(
            From: new CityRoutePointRequest("Anchor", fromId),
            To: new CityRoutePointRequest("Building", toId),
            Profile: "Pedestrian");
    }

    public static CityRouteDto CreateRouteDto(Guid cityId)
    {
        Guid districtId = Guid.Parse("2be5fd6f-c7a4-41b0-87da-73e7048919a8");
        Guid roadNodeId = Guid.Parse("715942c3-f84a-49cc-a96c-0b344a98f9c2");
        ...
        return new CityRouteDto(
            ...
            EffectiveTickId: 42,
            ConditionsLastEvaluatedAtUtc: new DateTimeOffset(2048, 6, 1, 9, 30, 0, TimeSpan.Zero),
    ...
    public static DispatchCityTripRequest CreateDispatchCityTripRequest(Guid fromId, Guid toId)
    {
    ...
    public static CityActiveTripDto CreateActiveTripDto(Guid cityId)
    {
        Guid districtId = Guid.Parse("2be5fd6f-c7a4-41b0-87da-73e7048919a8");
        Guid roadNodeId = Guid.Parse("715942c3-f84a-49cc-a96c-0b344a98f9c2");
        Guid roadSegmentId = Guid.Parse("99fd54d6-9a4b-4f35-8d74-b75ac4163bff");
        ...
        return new CityActiveTripDto(
            ...
            StartedAtSimTimeUtc: new DateTimeOffset(2048, 6, 1, 9, 30, 0, TimeSpan.Zero),
            LastAdvancedAtSimTimeUtc: new DateTimeOffset(2048, 6, 1, 9, 32, 0, TimeSpan.Zero),
            ExpectedArrivalAtSimTimeUtc: new DateTimeOffset(2048, 6, 1, 9, 36, 0, TimeSpan.Zero),
    ...
    public static IConfiguration BuildValidApiConfiguration()
    {
        ...
        {
            ["ConnectionStrings:SimulationCoreDb"] = "Host=localhost;Port=5432;Database=simulationcore_tests;Username=postgres;Password=postgres",
    ...
    public static TProperty GetAnonymousProperty<TProperty>(IResult result, string propertyName, int expectedStatusCode)
    {
        IStatusCodeHttpResult status = Assert.IsAssignableFrom<IStatusCodeHttpResult>(result);
        Assert.Equal(expectedStatusCode, status.StatusCode);
