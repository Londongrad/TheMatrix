...
public sealed class RetryCityPopulationBootstrapProvisioningTests
{
    ...
    {
        var result = _validator.Validate(new RetryCityPopulationBootstrapProvisioningCommand(Guid.NewGuid(), 25_000));
    ...
    {
        var result = _validator.Validate(new RetryCityPopulationBootstrapProvisioningCommand(Guid.Empty, 1_000_001));

        Assert.False(result.IsValid);
        Assert.Contains(result.Errors, error => error.PropertyName == "CityId");
        Assert.Contains(result.Errors, error => error.PropertyName == "PlannedPeopleCountOverride");
    }
    ...
    public async Task Handle_WhenRestartReturnsNotFound_ReturnsNotFound()
    {
        ...
        var handler = new RetryCityPopulationBootstrapProvisioningCommandHandler(mediator, orchestrator);
        var command = new RetryCityPopulationBootstrapProvisioningCommand(Guid.NewGuid(), 25_000);

        var result = await handler.Handle(command, CancellationToken.None);

        var restartCommand = Assert.IsType<RestartCityPopulationBootstrapCommand>(mediator.RequestedRequest);
        Assert.Equal(command.CityId, restartCommand.CityId);
        Assert.Equal(command.PlannedPeopleCountOverride, restartCommand.PlannedPeopleCountOverride);
        Assert.Equal(RetryCityPopulationBootstrapProvisioningStatus.NotFound, result.Status);
    ...
    public async Task Handle_WhenRestartReturnsNotAllowed_ReturnsNotAllowed()
    {
        ...
        var result = await handler.Handle(
            new RetryCityPopulationBootstrapProvisioningCommand(Guid.NewGuid(), 25_000),
            CancellationToken.None);

        Assert.Equal(RetryCityPopulationBootstrapProvisioningStatus.NotAllowed, result.Status);
        Assert.Null(result.Provisioning);
        Assert.Null(orchestrator.RequestedCityId);
    }
    ...
    public async Task Handle_WhenRestartSucceeds_ReturnsAcceptedProvisioningView()
    {
        ...
        var result = await handler.Handle(
            new RetryCityPopulationBootstrapProvisioningCommand(cityId, 25_000),
            CancellationToken.None);

        Assert.Equal(cityId, orchestrator.RequestedCityId);
        Assert.Equal(RetryCityPopulationBootstrapProvisioningStatus.Accepted, result.Status);
        Assert.Equal(provisioning, result.Provisioning);
    }
    ...
    private sealed class FakeClassicCityProvisioningOrchestrator : IClassicCityProvisioningOrchestrator
    {
        ...
        public Task<CityProvisioningModel> CreateAsync(
            Matrix.SimulationCore.Application.Scenarios.ClassicCity.UseCases.Cities.CreateCity.CreateCityCommand request,
