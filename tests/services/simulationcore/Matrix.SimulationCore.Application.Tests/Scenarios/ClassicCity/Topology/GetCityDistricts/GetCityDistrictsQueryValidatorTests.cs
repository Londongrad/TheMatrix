...
namespace Matrix.SimulationCore.Application.Tests.Scenarios.ClassicCity.Topology.GetCityDistricts
{
    public sealed class GetCityDistrictsQueryValidatorTests
    {
        ...
        {
            ValidationResult? result = _validator.Validate(new GetCityDistrictsQuery(Guid.NewGuid()));
        ...
        {
            ValidationResult? result = _validator.Validate(new GetCityDistrictsQuery(Guid.Empty));
