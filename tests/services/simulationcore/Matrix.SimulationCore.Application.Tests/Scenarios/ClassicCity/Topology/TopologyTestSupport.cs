    ...
    internal static class TopologyTestSupport
    {
            ...
            string name = "Central District")
        {
            return District.Create(
                cityId: cityId ?? new CityId(Guid.NewGuid()),
            ...
            RoadNodeId? accessRoadNodeId = null)
        {
            return CityAnchor.Create(
                cityId: cityId ?? new CityId(Guid.NewGuid()),
                districtId: districtId ?? new DistrictId(Guid.NewGuid()),
                accessRoadNodeId: accessRoadNodeId ?? RoadNodeId.New(),
            ...
            string name = "Main Connector")
        {
            return RoadSegment.Create(
                cityId: cityId ?? new CityId(Guid.NewGuid()),
                districtId: districtId ?? new DistrictId(Guid.NewGuid()),
                fromRoadNodeId: fromRoadNodeId ?? RoadNodeId.New(),
                toRoadNodeId: toRoadNodeId ?? RoadNodeId.New(),
                name: name,
        ...
        internal sealed class FakeDistrictRepository : IDistrictRepository
        {
            ...
            public Task<IReadOnlyList<District>> ListByCityIdAsync(
                CityId cityId,
                CancellationToken cancellationToken)
            {
                RequestedCityId = cityId;
        ...
        internal sealed class FakeResidentialBuildingRepository : IResidentialBuildingRepository
        {
            ...
            public IReadOnlyList<ResidentialBuilding> AddedBuildings { get; private set; } =
                Array.Empty<ResidentialBuilding>();
