...
public sealed class ResolveCityRoutesBatchQueryHandlerTests
{
    ...
    private static readonly DateTimeOffset CreatedAtUtc = new(2030, 1, 1, 0, 0, 0, TimeSpan.Zero);
    ...
    public void Validate_WhenBatchIsEmpty_ReturnsError()
    {
        ...
        Assert.Contains(result.Errors, x => x.PropertyName == "Routes");
    }
    ...
    {
        TestTopology topology = CreateTopology(buildingCount: 2, anchorCount: 2);
        ...
        ResolveCityRoutesBatchResult result = await handler.Handle(
            request: new ResolveCityRoutesBatchQuery(
                ...
                Routes:
                [
                    CreateQueryItem(0, topology.Buildings[0].Id.Value, topology.Anchors[0].Id.Value),
                    CreateQueryItem(1, topology.Buildings[1].Id.Value, topology.Anchors[1].Id.Value)
                ]),
            cancellationToken: CancellationToken.None);

        Assert.Equal(2, result.Routes.Count);
        Assert.Equal(0, result.Routes[0].Index);
        Assert.Equal(1, result.Routes[1].Index);
        Assert.Equal(1, roadNodeRepository.ListCalls);
        Assert.Equal(1, roadSegmentRepository.ListCalls);
        Assert.Equal(1, residentialBuildingRepository.ListCalls);
        Assert.Equal(0, residentialBuildingRepository.GetByIdCalls);
        Assert.Equal(1, cityAnchorRepository.ListCalls);
        Assert.Equal(0, cityAnchorRepository.GetByIdCalls);
        Assert.Equal(1, conditionsClient.GetByCityIdCalls);
    }

    [Fact]
    public async Task Handle_PreservesInputOrder()
    {
        TestTopology topology = CreateTopology(buildingCount: 3, anchorCount: 3);
        ...
        ResolveCityRoutesBatchResult result = await handler.Handle(
            request: new ResolveCityRoutesBatchQuery(
                ...
                Routes:
                [
                    CreateQueryItem(0, topology.Buildings[2].Id.Value, topology.Anchors[2].Id.Value),
                    CreateQueryItem(1, topology.Buildings[0].Id.Value, topology.Anchors[0].Id.Value),
                    CreateQueryItem(2, topology.Buildings[1].Id.Value, topology.Anchors[1].Id.Value)
                ]),
            cancellationToken: CancellationToken.None);

        Assert.Equal(3, result.Routes.Count);
        Assert.Equal(0, result.Routes[0].Index);
        Assert.Equal(topology.Buildings[2].Id.Value, result.Routes[0].Route!.From.EntityId);
        Assert.Equal(1, result.Routes[1].Index);
        Assert.Equal(topology.Buildings[0].Id.Value, result.Routes[1].Route!.From.EntityId);
        Assert.Equal(2, result.Routes[2].Index);
        Assert.Equal(topology.Buildings[1].Id.Value, result.Routes[2].Route!.From.EntityId);
    }

    [Fact]
    public async Task Handle_WhenPointCannotBeResolved_ReturnsNullRouteForThatItem()
    {
        TestTopology topology = CreateTopology(buildingCount: 1, anchorCount: 1);
        ResolveCityRoutesBatchQueryHandler handler = CreateHandler(topology);
        ...
        ResolveCityRoutesBatchResult result = await handler.Handle(
            request: new ResolveCityRoutesBatchQuery(
                ...
                [
                    CreateQueryItem(0, topology.Buildings[0].Id.Value, topology.Anchors[0].Id.Value),
                    CreateQueryItem(1, Guid.Parse("99999999-9999-9999-9999-999999999999"), topology.Anchors[0].Id.Value)
                ]),
            cancellationToken: CancellationToken.None);
    ...
    public async Task Handle_DeduplicatesIdenticalRoutePlanning()
    {
        ...
        Assert.Equal(1, routePlanner.PlanCalls);
    }
        ...
        int anchorCount)
    {
        ...
        for (int i = 0; i < buildingCount; i++)
            buildings.Add(CreateResidentialBuilding(
                index: i + 1,
                accessRoadNodeId: roadNodes[i].Id));

        List<CityAnchor> anchors = [];
        for (int i = 0; i < anchorCount; i++)
            anchors.Add(CreateCityAnchor(
                index: i + 1,
                accessRoadNodeId: roadNodes[i].Id));
