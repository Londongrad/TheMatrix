...
public sealed class MySessionsControllerTests
{
    ...
    {
        Guid currentSessionId = Guid.Parse("846bc0af-f5d0-434b-a7e0-a285dbe2b3a5");
        Guid otherSessionId = Guid.Parse("63191635-397a-4fd3-9697-4fe7d6b44b5a");
        var sender = new FakeSender();
        sender.Handle<GetMySessionsQuery, IReadOnlyCollection<MySessionResult>>(_ =>
            [
                CreateMySessionResult(sessionId: currentSessionId, isActive: true),
                CreateMySessionResult(sessionId: otherSessionId, isActive: false, isPersistent: false, ipAddress: null)
            ]);
    ...
    {
        Guid sessionId = Guid.Parse("15ae2a1c-5002-4129-b6d6-1d21eaf5274e");
        var sender = new FakeSender();
        sender.Handle<GetMySessionHistoryPageQuery, PagedResult<MySessionResult>>(query =>
            new PagedResult<MySessionResult>(
                items: [CreateMySessionResult(sessionId: sessionId, isActive: true)],
                totalCount: 11,
        ...
        Assert.False(response.Items.Single().IsActive);
        Assert.False(response.Items.Single().IsCurrent);
    }

    [Fact]
    public async Task RevokeEndpoints_ReturnNoContentAndSendCommands()
    {
        Guid sessionId = Guid.Parse("3c64f1c4-e2b2-4b99-a35c-577cd7f8c31c");
        var sender = new FakeSender();
        ...
        IActionResult revokeSessionResult = await controller.RevokeSession(sessionId, CancellationToken.None);
        ...
        Assert.Collection(
            sender.Requests,
            request => Assert.Equal(sessionId, Assert.IsType<RevokeMySessionCommand>(request).SessionId),
