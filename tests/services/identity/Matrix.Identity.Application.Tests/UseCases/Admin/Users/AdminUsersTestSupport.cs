...
namespace Matrix.Identity.Application.Tests.UseCases.Admin.Users
{
    internal static class AdminUsersTestSupport
    {
        ...
        internal sealed class FakeUserSessionRepository : IUserSessionRepository
        {
            ...
            {
                RequestedUserId = userId;
                IReadOnlyCollection<UserSession> items = Sessions.Where(x => x.UserId == userId)
                   .ToArray();
        ...
        {
            public IReadOnlyCollection<UserPermissionOverrideResult> GetUserPermissionsResult { get; set; } =
                Array.Empty<UserPermissionOverrideResult>();
            ...
            public Task<IReadOnlyCollection<UserPermissionOverrideResult>> GetUserPermissionsAsync(
                Guid userId,
                CancellationToken cancellationToken)
            {
                RequestedUserId = userId;
            ...
            public Task<bool> UpsertUserPermissionAsync(
                Guid userId,
            ...
            public Task<bool> ReplaceUserPermissionsAsync(
                Guid userId,
                IReadOnlyDictionary<string, PermissionEffect> permissionEffects,
                CancellationToken cancellationToken)
            {
                RequestedUserId = userId;
                ReplacedPermissionEffects = new Dictionary<string, PermissionEffect>(
        ...
        {
            public IReadOnlyCollection<PermissionCatalogItemResult> Permissions { get; set; } =
                Array.Empty<PermissionCatalogItemResult>();

            public Dictionary<string, PermissionCatalogItemResult> PermissionByKey { get; } =
                new(StringComparer.Ordinal);

            public string? RequestedPermissionKey { get; private set; }

            public Task<IReadOnlyCollection<PermissionCatalogItemResult>> GetPermissionsAsync(
                CancellationToken cancellationToken)
            {
            ...
            {
                RequestedPermissionKey = permissionKey;
                PermissionByKey.TryGetValue(
                    key: permissionKey,
                    value: out PermissionCatalogItemResult? permission);
        ...
        internal sealed class FakeAdminUserGuard : IAdminUserGuard
        {
            ...
            public Task EnsureUserCanBeManagedAsync(
                Guid targetUserId,
                CancellationToken cancellationToken)
            {
            ...
            public Task EnsureRoleAssignmentIsAllowedAsync(
        ...
        internal sealed class FakeDefaultUserAccessPolicyRepository : IDefaultUserAccessPolicyRepository
        {
            ...
            public IReadOnlyDictionary<string, PermissionEffect> OverridesResult { get; set; } =
                new Dictionary<string, PermissionEffect>(StringComparer.Ordinal);

            public bool ReplaceResult { get; set; }

            public DefaultUserAccessPolicy PolicyForUpdate { get; set; } =
                DefaultUserAccessPolicy.CreateDefault(UtcNow.AddDays(-1));
            ...
            public Task<IReadOnlyDictionary<string, PermissionEffect>> GetOverridesAsync(
                CancellationToken cancellationToken)
            {
                return Task.FromResult(OverridesResult);
            }

            public Task<bool> ReplaceOverridesAsync(
                IReadOnlyDictionary<string, PermissionEffect> overrides,
                CancellationToken cancellationToken)
            {
                ReplacedOverrides = new Dictionary<string, PermissionEffect>(
