...
namespace Matrix.Identity.Application.Tests.UseCases.Self.Account
{
    public sealed class AccountCommandValidatorTests
    {
        ...
        public void ChangeDisplayNameValidator_WithNullOrShortDisplayName_ReturnsNoErrors()
        {
            ...
            ValidationResult? nullResult = validator.Validate(new ChangeDisplayNameCommand(DisplayName: null));
            ValidationResult? shortResult = validator.Validate(new ChangeDisplayNameCommand(DisplayName: "Neo"));
        ...
        public void ChangeUsernameValidator_WithMissingPasswordAndShortUsername_ReturnsExpectedMessages()
        {
            ...
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "CurrentPassword" && x.ErrorMessage == "Current password is required.");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "Username" &&
                             x.ErrorMessage == $"Username must be at least {Username.MinLength} characters long.");
        }
    }
}
