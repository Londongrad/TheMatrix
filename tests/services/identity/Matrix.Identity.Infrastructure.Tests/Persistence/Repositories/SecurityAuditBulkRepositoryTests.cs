...
public sealed class SecurityAuditBulkRepositoryTests
{
    ...
    public async Task DeleteBatchAsync_UsesProviderAwareSqlAndDeletesOldestRowsFirst()
    {
        ...
        database.DbContext.SecurityAuditEvents.AddRange(
            CreateSecurityAuditRecord(subject: "first", occurredAtUtc: CreatedAtUtc),
            CreateSecurityAuditRecord(subject: "second", occurredAtUtc: CreatedAtUtc.AddMinutes(10)),
            CreateSecurityAuditRecord(subject: "third", occurredAtUtc: CreatedAtUtc.AddMinutes(20)));
