    ...
    GetCityDistrictSanitationConditions
{
    public sealed class GetCityDistrictSanitationConditionsQueryHandlerTests
    {
        ...
        public async Task Handle_WhenStateDoesNotExist_ReturnsNull()
        {
            ...
            GetCityDistrictSanitationConditionsQueryHandler handler = CreateHandler(
                repository: repository,
                topologyClient: topologyClient);

            CityDistrictSanitationConditionsDto? result = await handler.Handle(
                request: new GetCityDistrictSanitationConditionsQuery(SimulationSystemsApplicationTestSupport.CityId),
        ...
        public async Task Handle_WhenStateAndTopologyExist_ReturnsProjectedDistrictConditions()
        {
            ...
            var handler = new GetCityDistrictSanitationConditionsQueryHandler(
                ...
                projectionPolicy: new ClassicCityDistrictSanitationProjectionPolicy());
            ...
            Assert.All(
                ...
                {
                    Assert.InRange(
                        actual: district.SanitationCoverageIndex,
                        low: 0m,
                        high: 1m);
                    Assert.InRange(
                        actual: district.SanitationSupportIndex,
                        low: 0m,
                        high: 1m);
                    Assert.InRange(
                        actual: district.OverflowRiskIndex,
                        low: 0m,
                        high: 1m);
                    Assert.InRange(
                        actual: district.ContaminationRiskIndex,
                        low: 0m,
                        high: 1m);
                    Assert.InRange(
