...
public sealed class CityEnvironmentalSyncConsumersTests
{
    ...
    public async Task CityWeatherCreatedConsumeAsync_WhenApplied_SendsMappedCommandAndLogsInformation()
    {
        ...
        Assert.Equal(message.AtSimTimeUtc, command.AtSimTimeUtc);
        Assert.Equal(message.InitialState.Type, command.Weather.Type);
        TestLogEntry entry = Assert.Single(logger.Entries);
        Assert.Equal(LogLevel.Information, entry.LogLevel);
        Assert.Contains("weather-initialization", entry.Message);
    }
    ...
    public async Task CityWeatherChangedConsumeAsync_WhenStale_LogsWarning()
    {
        ...
        await consumer.ConsumeAsync(CreateWeatherChangedMessage(), CancellationToken.None);

        TestLogEntry entry = Assert.Single(logger.Entries);
        Assert.Equal(LogLevel.Warning, entry.LogLevel);
        Assert.Contains("Skipped stale classic city environmental weather sync", entry.Message);
    }
    ...
    public async Task CityStockpileSnapshotConsumeAsync_WhenDeferred_LogsDebug()
    {
        ...
        var consumer = new CityStockpileSnapshotConsumer(mediator, logger);

        await consumer.ConsumeAsync(CreateStockpileSnapshotMessage(), CancellationToken.None);

        TestLogEntry entry = Assert.Single(logger.Entries);
        Assert.Equal(LogLevel.Debug, entry.LogLevel);
        Assert.Contains("Deferred classic city resource supply snapshot", entry.Message);
    }
    ...
    private static CityWeatherCreatedV1 CreateWeatherCreatedMessage()
    {
        return new CityWeatherCreatedV1(
            ...
            InitialState: CreateWeatherState("Snowstorm", "Severe"),
    ...
    private static ClassicCityStockpileSnapshotV1 CreateStockpileSnapshotMessage()
    {
        ...
        return new ClassicCityStockpileSnapshotV1(
            ...
            Fuel: line,
            Food: line with { Kind = "Food" },
            Medicine: line with { Kind = "Medicine" },
            SpareParts: line with { Kind = "SpareParts" },
            Filters: line with { Kind = "Filters" },
            EmergencyWater: line with { Kind = "EmergencyWater" },
            EffectiveTickId: 12,
            EffectiveAtUtc: LaterUtc,
    ...
    private sealed class TestRecalculateMediator : IMediator
    {
        ...
        {
            RecalculateCityEnvironmentalConditionsCommand command = Assert.IsType<RecalculateCityEnvironmentalConditionsCommand>(request);
