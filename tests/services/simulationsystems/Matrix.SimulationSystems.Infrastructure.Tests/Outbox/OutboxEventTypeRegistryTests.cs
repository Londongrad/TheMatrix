    ...
    public sealed class OutboxEventTypeRegistryTests
    {
            ...
            var registry = new OutboxEventTypeRegistry(
                ...
                [
                    new StubContributor(
                        ...
                        {
                            ["simulation-systems.test-event.v1"] = typeof(TestEvent)
                        })
                ]);

            Assert.Equal(
                expected: typeof(TestEvent),
                actual: registry.Resolve("simulation-systems.test-event.v1"));
            Assert.Equal(
                expected: 1,
        ...
        public void Constructor_WhenContributorsRepeatEventType_ThrowsInvalidOperationException()
        {
            ...
            [
                ...
                new StubContributor(
                    new Dictionary<string, Type>
                    {
                        ["simulation-systems.test-event.v1"] = typeof(OtherTestEvent)
                    })
            ];
            ...
            Assert.Contains(
                expectedSubstring: "simulation-systems.test-event.v1",
        ...
        public void Resolve_WhenEventTypeIsUnknown_ThrowsNotSupportedException()
        {
            ...
            NotSupportedException exception = Assert.Throws<NotSupportedException>(
                () => registry.Resolve("simulation-systems.unknown.v1"));

            Assert.Contains(
                expectedSubstring: "simulation-systems.unknown.v1",
                actualString: exception.Message);
        }
