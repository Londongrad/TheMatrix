...
public sealed class CityPopulationHealthcarePressurePolicyTests
{
    ...
    public void Evaluate_WhenNoAliveResidents_ReturnsBaselineProfile()
    {
        ...
        Matrix.Population.Domain.Scenarios.ClassicCity.Models.CityPopulationHealthcarePressureProfile profile = policy.Evaluate(
            residents: [deceasedResident]);

        Assert.Equal(0, profile.ActiveIllnessCount);
        Assert.Equal(0, profile.SevereIllnessCount);
        Assert.Equal(0.20m, profile.MedicalLoadIndex);
        Assert.Equal(0m, profile.TriagePressureIndex);
        Assert.Equal(1m, profile.RecoverySupportIndex);
    }
    ...
    public void Evaluate_WhenResidentsHaveMixedIllnesses_ReturnsExpectedCountsAndClampedIndexes()
    {
        ...
        Matrix.Population.Domain.Entities.Person mildCase = PopulationTestData.CreateAdultPerson();
        mildCase.DiagnoseIllness(
            kind: IllnessKind.Infection,
            severity: IllnessSeverity.Mild,
            currentDate: currentDate);

        Matrix.Population.Domain.Entities.Person severeCase = PopulationTestData.CreateAdultPerson(
            firstName: "Olga",
            lastName: "Ivanova",
            sex: Sex.Female,
        ...
        Matrix.Population.Domain.Entities.Person healthyResident = PopulationTestData.CreateAdultPerson(
            firstName: "Petr",
            lastName: "Sidorov",
            personId: Guid.Parse("99999999-2222-2222-2222-222222222222"));

        Matrix.Population.Domain.Scenarios.ClassicCity.Models.CityPopulationHealthcarePressureProfile profile = policy.Evaluate(
            residents: [mildCase, severeCase, healthyResident]);

        Assert.Equal(2, profile.ActiveIllnessCount);
        Assert.Equal(1, profile.SevereIllnessCount);
        Assert.Equal(3m, profile.MedicalLoadIndex);
        Assert.Equal(3m, profile.TriagePressureIndex);
        Assert.Equal(0.25m, profile.RecoverySupportIndex);
    }
    ...
    public void Evaluate_WhenServiceQualityAndSuppliesDegrade_ProducesHigherLoadAndLowerRecovery()
    {
        ...
        Matrix.Population.Domain.Scenarios.ClassicCity.Models.CityPopulationHealthcarePressureProfile supportedProfile = policy.Evaluate(
            residents: [mildCase, healthyResident],
        ...
        Matrix.Population.Domain.Scenarios.ClassicCity.Models.CityPopulationHealthcarePressureProfile degradedProfile = policy.Evaluate(
            residents: [mildCase, healthyResident],
