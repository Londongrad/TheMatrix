...
public sealed class ResidentHouseholdPressureProgressionStepTests
{
    ...
    private static readonly DateOnly PreviousDate = new(2048, 5, 5);
    ...
    public async Task ApplyAsync_WhenHouseholdResidentsAreMissing_ReturnsFalse()
    {
        ...
        Assert.Equal(initial, Capture(resident));
    ...
    public async Task ApplyAsync_WhenHouseholdResidentsHaveCommuteDestinations_BuildsCommuteProfileFromRouting()
    {
        ...
        Assert.Equal(workplaceAnchorId, request.DestinationAnchorId);
        Assert.Equal(CityPopulationCommuteRoutingProfiles.Pedestrian, request.Profile);
        Assert.Equal([workplaceAnchorId], routingService.ResolvedAnchorIds);
    }
        ...
        DateOnly? currentDate = null)
    {
        return ResidentHouseholdPressureProgressionStep.ApplyAsync(
            cityId: TestCityId,
            person: resident,
            residentsByHouseholdId: residentsByHouseholdId ?? new Dictionary<HouseholdId, IReadOnlyCollection<Person>>(),
    ...
    {
        var evaluatedAtUtc = new DateTimeOffset(2048, 5, 6, 0, 0, 0, TimeSpan.Zero);
