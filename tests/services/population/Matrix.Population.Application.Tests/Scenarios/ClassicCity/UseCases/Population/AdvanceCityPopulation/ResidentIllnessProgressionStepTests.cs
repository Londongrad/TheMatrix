...
namespace Matrix.Population.Application.Tests.Scenarios.ClassicCity.UseCases.Population.AdvanceCityPopulation
{
    public sealed class ResidentIllnessProgressionStepTests
    {
        ...
        public async Task ApplyAsync_WhenCurrentDateDoesNotAdvance_ReturnsFalseButStillResolvesHealthcareContext()
        {
            ...
            Assert.Equal(
                expected: 1,
                actual: routingService.HealthcareResolveCallCount);
        }

        [Fact]
        public async Task
            ApplyAsync_WhenHospitalAnchorAndResidentialBuildingExist_ResolvesHealthcareCommuteWithSelectedAnchor()
        {
            Person resident = CreatePerson(currentDate: CurrentDate);
            var healthcareAnchorId = CityAnchorId.From(Guid.Parse("dddddddd-dddd-dddd-dddd-dddddddddddd"));
            var routingService = new RecordingCommuteRoutingService();
            ...
            await ApplyAsync(
                ...
                hospitalAnchors:
                [
                    CreateHospitalAnchor(
                        districtId: TestDistrictId,
                        anchorId: healthcareAnchorId)
                ],
                commuteRoutingService: routingService);
            ...
            Assert.Equal(
                ...
                actual: routingService.LastHealthcareCityId);
            Assert.Equal(
                expected: TestResidentialBuildingId,
                actual: routingService.LastHealthcareResidentialBuildingId);
            Assert.Equal(
                expected: healthcareAnchorId,
                actual: routingService.LastHealthcareAnchorId);
        }
        ...
        public async Task ApplyAsync_WhenResidentHasSevereInfectionAndLowSupport_AppliesIllnessBurden()
        {
            ...
            var before = IllnessSnapshot.Capture(resident);
            var routingService = new RecordingCommuteRoutingService
            {
        ...
        public async Task ApplyAsync_WhenAdverseExposureCanDiagnoseExposureIllness_DiagnosesExposureIllness()
        {
            ...
            Assert.True(resident.HasActiveIllness);
            Assert.Equal(
                expected: IllnessKind.Exposure,
                actual: resident.CurrentIllnessKind);
        }
        ...
        public async Task ApplyAsync_WhenResidentIsAlreadyDead_ReturnsFalse()
        {
            ...
            Assert.Equal(
                expected: before,
                actual: IllnessSnapshot.Capture(resident));
        ...
        {
            return new CityHealthcareAutonomyPolicy(householdLivelihoodPolicy: new CityHouseholdLivelihoodPolicy());
        }
