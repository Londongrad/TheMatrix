    ...
    public sealed class CityPopulationBootstrapTuningInputValidatorTests
    {
        ...
        public void Validate_WithValidInput_ReturnsNoErrors()
        {
            ValidationResult? result = _validator.Validate(
                new CityPopulationBootstrapTuningInput(
        ...
        public void Validate_WithOutOfRangeValues_ReturnsErrors()
        {
            ...
            Assert.Contains(
                ...
                filter: x => x.PropertyName == "HousingPressurePercent");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "EconomicStabilityPercent");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "SocialVolatilityPercent");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "FamilyFormationPercent");
        }
    }
}
