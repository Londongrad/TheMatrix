...
public sealed class CivilRegistryAutonomyStepTests
{
    ...
    {
        var first = CreateAdultResident(
            personId: CreateGuid(1),
            householdId: CreateGuid(101));
        var second = CreateAdultResident(
        ...
        int affected = await CivilRegistryAutonomyStep.ApplyAsync(
            cityId: TestCityId,
            residentsById: CreateResidentsById(first, second),
        ...
        Assert.Equal(MaritalStatus.Single, first.MaritalStatus);
        Assert.Equal(MaritalStatus.Single, second.MaritalStatus);
    }
    ...
    public async Task ApplyAsync_WhenMarriageDecisionIsPlanned_RegistersMarriageMergesHouseholdsAndWritesActivity()
    {
        ...
        var householdWriteRepository = new FakeHouseholdWriteRepository();
        ConfigureHousedHousehold(householdWriteRepository, first.HouseholdId, residentCount: 1);
        ConfigureHomelessHousehold(householdWriteRepository, second.HouseholdId, residentCount: 1);
        List<CityPopulationActivityWriteModel> activityEntries = [];
        ...
        Assert.Equal(MaritalStatus.Married, first.MaritalStatus);
        Assert.Equal(MaritalStatus.Married, second.MaritalStatus);
        Assert.Equal(second.Id, first.SpouseId);
        Assert.Equal(first.Id, second.SpouseId);
        ...
        Assert.Equal(first.HouseholdId, updatedTargetHousehold.Id);
        ...
        Assert.Equal(1, deletedSourceHousehold.Size.Value);

        CityPopulationActivityWriteModel activity = Assert.Single(activityEntries);
        Assert.Equal(CityPopulationActivityEventType.ResidentsMarried, activity.EventType);
        ...
        Assert.Equal(first.Id.Value, activity.PrimaryResidentId);
        Assert.Equal(second.Id.Value, activity.SecondaryResidentId);
    }
    ...
    public async Task ApplyAsync_WhenDivorceDecisionIsPlanned_RegistersDivorceSeparatesHouseholdsAndWritesActivity()
    {
        ...
        var householdWriteRepository = new FakeHouseholdWriteRepository();
        ConfigureHousedHousehold(householdWriteRepository, householdId, residentCount: 2);
        List<CityPopulationActivityWriteModel> activityEntries = [];
        ...
        Assert.NotEqual(householdId, second.HouseholdId);
        HouseholdEntity updatedSharedHousehold = Assert.Single(householdWriteRepository.UpdatedHouseholds);
        Assert.Equal(householdId, updatedSharedHousehold.Id);
        ...
        Assert.Equal(second.HouseholdId, newHousehold.Id);
        Assert.Equal(1, newHousehold.Size.Value);
        Assert.Equal(OccurredAtUtc, newHousehold.CreatedAtUtc);
        Assert.Equal(HousingStatus.Housed, newPlacement.HousingStatus);
        Assert.Equal(CreateDistrictId(householdId), newPlacement.DistrictId);
        Assert.Equal(CreateResidentialBuildingId(householdId), newPlacement.ResidentialBuildingId);

        CityPopulationActivityWriteModel activity = Assert.Single(activityEntries);
        Assert.Equal(CityPopulationActivityEventType.ResidentsDivorced, activity.EventType);
        Assert.Equal(CityPopulationActivitySource.Autonomy, activity.Source);
        ...
        CityCivilRegistryAutonomyPolicy policy)
    {
        ...
        {
            PersonEntity[] residents = Enumerable.Range(0, 10)
        ...
        CityCivilRegistryAutonomyPolicy policy)
    {
        for (int seed = 1; seed <= 2_000; seed++)
        {
            ...
            HouseholdId householdId = HouseholdId.From(CreateGuid(60_000 + seed));
    ...
    private static void ConfigureHousedHousehold(
        ...
        int residentCount)
    {
        repository.HouseholdsById[householdId] = CreateHousehold(householdId, residentCount);
    ...
    {
        return DistrictId.From(Guid.Parse($"11111111-1111-1111-1111-{Math.Abs(householdId.Value.GetHashCode()) % 1_000_000_000_000:000000000000}"));
    }

    private static ResidentialBuildingId CreateResidentialBuildingId(HouseholdId householdId)
    {
        return ResidentialBuildingId.From(Guid.Parse($"22222222-2222-2222-2222-{Math.Abs(householdId.Value.GetHashCode()) % 1_000_000_000_000:000000000000}"));
    }
