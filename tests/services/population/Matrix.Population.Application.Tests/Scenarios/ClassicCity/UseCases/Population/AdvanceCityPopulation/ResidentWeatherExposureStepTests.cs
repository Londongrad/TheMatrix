...
public sealed class ResidentWeatherExposureStepTests
{
    ...
    private static readonly DateTimeOffset DayStartUtc = new(2048, 7, 10, 0, 0, 0, TimeSpan.Zero);
    ...
    public void Apply_WhenExposureSegmentsAreEmpty_ReturnsFalseAndDoesNotChangeResident()
    {
        ...
        WeatherSnapshot before = WeatherSnapshot.Capture(resident);
        ...
        Assert.Equal(before, WeatherSnapshot.Capture(resident));
    ...
    public void Apply_WhenMultipleSegmentsHaveImpacts_AggregatesDeltasBeforeApplying()
    {
        ...
        bool changed = Apply(
            resident: resident,
            exposureSegments: [adverseSegment, recoverySegment],
            policy: policy);

        Assert.True(adverseImpact.HasEffect);
        Assert.True(recoveryImpact.HasEffect);
        Assert.True(changed);
        Assert.Equal(before.Health + adverseImpact.HealthDelta + recoveryImpact.HealthDelta, resident.Health.Value);
        Assert.Equal(
            before.Happiness + adverseImpact.HappinessDelta + recoveryImpact.HappinessDelta,
            resident.Happiness.Value);
    }
    ...
    public void Apply_WhenEnvironmentReducesWeatherImpact_UsesEnvironmentInCalculation()
    {
        ...
        Assert.NotEqual(baselineImpact, adaptedImpact);
    ...
    public void Apply_WhenHealthDeltaKillsResident_DoesNotApplyHappinessDelta()
    {
        ...
        Assert.True(changed);
        Assert.False(resident.IsAlive);
        Assert.Equal(previousHappiness, resident.Happiness.Value);
    }
        ...
        DateTimeOffset? intervalEndUtc = null)
    {
        return new CityWeatherExposureSegment(
            ...
            IntervalEndSimTimeUtc: intervalEndUtc ?? DayStartUtc.AddDays(1).AddHours(6),
