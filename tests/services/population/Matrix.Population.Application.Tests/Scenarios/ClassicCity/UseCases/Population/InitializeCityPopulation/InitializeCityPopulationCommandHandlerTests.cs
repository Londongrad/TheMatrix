...
public sealed class InitializeCityPopulationCommandHandlerTests
{
    ...
    public async Task Handle_WhenCityIsArchived_ThrowsConflictWithoutMutatingState()
    {
        ...
        Assert.Equal("Population.City.Archived", exception.Code);
        ...
        Assert.Equal(0, anchorCatalogRepository.DeleteByCityCalls);
    ...
    public async Task Handle_WhenCityIsDeleted_ThrowsConflictWithoutMutatingState()
    {
        ...
        Assert.Equal("Population.City.Deleted", exception.Code);
        Assert.Equal(ApplicationErrorType.Conflict, exception.ErrorType);
        Assert.Equal(CityId.From(cityId), archiveStateRepository.RequestedCityId);
        Assert.Equal(CityId.From(cityId), deletionStateRepository.RequestedCityId);
        Assert.Equal(0, unitOfWork.ExecuteTransactionCalls);
