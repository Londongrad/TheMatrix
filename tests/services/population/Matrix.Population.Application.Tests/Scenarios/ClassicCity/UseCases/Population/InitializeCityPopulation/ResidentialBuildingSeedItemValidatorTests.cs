...
public sealed class ResidentialBuildingSeedItemValidatorTests
{
    ...
    {
        var result = _validator.Validate(new ResidentialBuildingSeedItem(
    ...
    public void Validate_WithInvalidFields_ReturnsErrors()
    {
        ...
        Assert.Contains(result.Errors, x => x.PropertyName == "ResidentialBuildingId");
        Assert.Contains(result.Errors, x => x.PropertyName == "DistrictId");
        Assert.Contains(result.Errors, x => x.PropertyName == "ResidentCapacity");
    }
}
