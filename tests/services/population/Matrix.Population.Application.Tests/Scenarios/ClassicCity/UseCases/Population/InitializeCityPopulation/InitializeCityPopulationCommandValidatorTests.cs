...
namespace Matrix.Population.Application.Tests.Scenarios.ClassicCity.UseCases.Population.InitializeCityPopulation
{
    public sealed class InitializeCityPopulationCommandValidatorTests
    {
        ...
        public void Validate_WithInvalidPeopleCount_ReturnsError()
        {
            ...
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "PeopleCount");
        }
        ...
        public void Validate_WithInvalidNestedItems_ReturnsNestedErrors()
        {
            ...
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "Environment.ClimateZone");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "Environment.Hemisphere");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "Environment.UtcOffsetMinutes");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "Tuning.HousingPressurePercent");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "Tuning.EconomicStabilityPercent");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "CityAnchors[0].CityAnchorId");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "CityAnchors[0].CreatedAtUtc");
            Assert.Contains(
                collection: result.Errors,
                filter: x => x.PropertyName == "ResidentialBuildings[0].ResidentCapacity");
        }
