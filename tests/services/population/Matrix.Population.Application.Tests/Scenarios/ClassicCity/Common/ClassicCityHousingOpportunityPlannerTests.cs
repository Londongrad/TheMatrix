...
public sealed class ClassicCityHousingOpportunityPlannerTests
{
    ...
    private static readonly DateOnly CurrentDate = new(2030, 1, 1);
    private static readonly DateTimeOffset CreatedAtUtc = new(2030, 1, 1, 0, 0, 0, TimeSpan.Zero);

    [Fact]
    public void BuildHousingOpportunityPool_FiltersHousedPlacementsWithDistrictAndBuildingAndPreservesFirstOccurrenceOrder()
    {
        ...
        Assert.Equal(firstDistrictId, pool[0].DistrictId);
        Assert.Equal(firstBuildingId, pool[0].ResidentialBuildingId);
        Assert.Equal(secondDistrictId, pool[1].DistrictId);
        Assert.Equal(secondBuildingId, pool[1].ResidentialBuildingId);
    }
    ...
    public void ResolveDistrictHousingStabilityContribution_CalculatesRoundedClampedScore()
    {
        ...
        Assert.InRange(result, low: 0m, high: 1m);
    }
    ...
    public void GetStableInt_IsDeterministicAndWithinRange()
    {
        ...
        Assert.InRange(first, low: 0, high: 11);
    }
    ...
    public void GetStableInt_WhenModulusIsNonPositive_ReturnsZero()
    {
        ...
        Assert.Equal(0, zeroModulusResult);
        Assert.Equal(0, negativeModulusResult);
    }
    ...
    public async Task SelectHousingOpportunityAsync_ChoosesHighestScoringCandidateInDeterministicWindow()
    {
        ...
        Assert.Equal(highScoreBuildingId, result.ResidentialBuildingId);
        Assert.Equal(3, routingService.AnchorRequests.Count);
        Assert.Contains(lowScoreBuildingId, routingService.AnchorRequests.Select(x => x.ResidentialBuildingId));
        Assert.Contains(highScoreBuildingId, routingService.AnchorRequests.Select(x => x.ResidentialBuildingId));
        Assert.Contains(mediumScoreBuildingId, routingService.AnchorRequests.Select(x => x.ResidentialBuildingId));
    }
    ...
    public async Task SelectHousingOpportunityAsync_RespectsCandidateWindowLimitOfTwelve()
    {
        ...
        Assert.Equal(12, evaluatedBuildings.Length);
        Assert.Contains(result.ResidentialBuildingId, evaluatedBuildings);
    }
    ...
    public void SelectHousingAnchorResident_PrefersAdultOrSeniorThenOlderAgeThenLowerPersonId()
    {
        ...
        Person adultOverChild = ClassicCityHousingOpportunityPlanner.SelectHousingAnchorResident(
            householdResidents: [child, adult],
            currentDate: CurrentDate);
        Person olderOverYounger = ClassicCityHousingOpportunityPlanner.SelectHousingAnchorResident(
            householdResidents: [youngerAdult, olderAdult],
            currentDate: CurrentDate);
        Person lowerIdOnTie = ClassicCityHousingOpportunityPlanner.SelectHousingAnchorResident(
            householdResidents: [higherIdAdult, lowerIdAdult],
            currentDate: CurrentDate);

        Assert.Equal(adult.Id, adultOverChild.Id);
        Assert.Equal(olderAdult.Id, olderOverYounger.Id);
        Assert.Equal(lowerIdAdult.Id, lowerIdOnTie.Id);
    }
    ...
    {
        return HouseholdId.From(CreateGuid(
            prefix: "10000000",
            index: index));
    }
    ...
    {
        return PersonId.From(CreateGuid(
            prefix: "20000000",
            index: index));
    }
    ...
    {
        return DistrictId.From(CreateGuid(
            prefix: "30000000",
            index: index));
    }
    ...
    {
        return ResidentialBuildingId.From(CreateGuid(
            prefix: "40000000",
            index: index));
    }
    ...
    {
        return CityAnchorId.From(CreateGuid(
            prefix: "50000000",
            index: index));
    }
