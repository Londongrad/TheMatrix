...
public sealed class ProcessedIntegrationMessageCleanupHostedServiceTests
{
    ...
    public async Task StartAsync_WhenCleanupIsDisabled_LogsInformationAndStops()
    {
        ...
        var service = CreateService(
            ...
            timeProvider: new FrozenTimeProvider(new DateTimeOffset(2048, 5, 3, 0, 0, 0, TimeSpan.Zero)),
    ...
    public async Task StartAsync_WhenBatchSizeIsNonPositive_LogsErrorAndStops()
    {
        ...
        TestLogEntry entry = Assert.Single(logger.Entries, x => x.LogLevel == LogLevel.Error);
        Assert.Contains("batch size must be > 0", entry.Message);
    }
    ...
    public async Task StartAsync_WhenTickOccurs_DeletesExpiredMarkersAndLogsDebug()
    {
        ...
        dbContext.ProcessedIntegrationMessages.AddRange(
            new ProcessedIntegrationMessage(
                ...
                processedAtUtc: new DateTimeOffset(2048, 5, 1, 20, 0, 0, TimeSpan.Zero)),
            new ProcessedIntegrationMessage(
                consumer: "Fresh",
                messageId: Guid.Parse("6d3794d1-c3c1-43cd-96b4-65f5c7453479"),
                processedAtUtc: new DateTimeOffset(2048, 5, 2, 23, 0, 0, TimeSpan.Zero)));
        ...
        TestLogEntry entry = Assert.Single(logger.Entries, x => x.LogLevel == LogLevel.Debug);
        Assert.Equal("Fresh", remaining.Consumer);
        Assert.Contains("Deleted 1 processed integration message markers", entry.Message);
        Assert.Contains("05/02/2048 00:00:00 +00:00", entry.Message);
    }
    ...
    public async Task StartAsync_WhenCleanupLoopThrows_LogsError()
    {
        ...
        await service.StartAsync(CancellationToken.None);
        await WaitUntilAsync(
            () => logger.Entries.Any(x => x.LogLevel == LogLevel.Error && x.Exception is not null),
            timeout: TimeSpan.FromSeconds(3));
        await service.StopAsync(CancellationToken.None);

        TestLogEntry entry = Assert.Single(logger.Entries, x => x.LogLevel == LogLevel.Error && x.Exception is not null);
        Assert.Contains("cleanup loop failed", entry.Message);
