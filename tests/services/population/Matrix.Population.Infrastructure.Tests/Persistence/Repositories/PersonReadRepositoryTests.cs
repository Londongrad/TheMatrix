...
public sealed class PersonReadRepositoryTests
{
    ...
    public async Task GetAllAsync_ReturnsStoredPersons()
    {
        ...
        dbContext.Persons.AddRange(first, second);
        ...
        Assert.Contains(persons, x => x.Id == first.Id);
        Assert.Contains(persons, x => x.Id == second.Id);
    }
    ...
    public async Task FindByIdAsync_ReturnsMatchingPerson()
    {
        ...
        Person? found = await repository.FindByIdAsync(PersonId.From(Guid.Parse("33333333-3333-3333-3333-333333333333")));

        Assert.NotNull(found);
        Assert.Equal(person.Id, found.Id);
        Assert.Equal("Vera", found.Name.FirstName);
    }
    ...
    public async Task GetPageAsync_ReturnsOrderedSliceAndTotalCount()
    {
        await using PopulationTestDatabase database = CreateDbContext();
        PopulationDbContext dbContext = database.DbContext;
        Person first = CreatePerson(personId: Guid.Parse("00000000-0000-0000-0000-000000000003"), firstName: "Third");
        Person second = CreatePerson(personId: Guid.Parse("00000000-0000-0000-0000-000000000001"), firstName: "First");
        Person third = CreatePerson(personId: Guid.Parse("00000000-0000-0000-0000-000000000002"), firstName: "Second");
        dbContext.Households.AddRange(
            CreateHousehold(householdId: first.HouseholdId.Value),
            CreateHousehold(householdId: second.HouseholdId.Value),
            CreateHousehold(householdId: third.HouseholdId.Value));
        dbContext.Persons.AddRange(first, second, third);
        await dbContext.SaveChangesAsync();
        var repository = new PersonReadRepository(dbContext);
        ...
        Assert.Equal("Second", item.Name.FirstName);
    }
}
