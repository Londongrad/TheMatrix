...
public static class ResourcesApiTestSupport
{
    ...
    private static readonly DateTimeOffset LastEvaluatedAtUtc = new(2051, 7, 11, 10, 15, 0, TimeSpan.Zero);

    public static CityStockpilesDto CreateCityStockpilesDto(Guid? cityId = null, bool emergencyRationingEnabled = false)
    {
        return new CityStockpilesDto(
            ...
            Fuel: CreateLineDto("Fuel", 0.62m, 0.48m, 0.75m, 0.22m),
            Food: CreateLineDto("Food", 0.73m, 0.31m, 0.80m, 0.18m),
            Medicine: CreateLineDto("Medicine", 0.69m, 0.35m, 0.71m, 0.20m),
            SpareParts: CreateLineDto("SpareParts", 0.58m, 0.54m, 0.66m, 0.29m),
            Filters: CreateLineDto("Filters", 0.64m, 0.40m, 0.70m, 0.24m),
            EmergencyWater: CreateLineDto("EmergencyWater", 0.67m, 0.38m, 0.72m, 0.21m));
    }
        ...
        Guid? cityId = null)
    {
        return new DispatchCityResupplyResult(
            ...
            PendingResupply: CreatePendingResupplyDto("Fuel", "Medium", 45),
            BudgetPressureIndex: 0.34m,
            BudgetAuthorizationStatus: status == DispatchCityResupplyStatus.AuthorizationDenied ? "Denied" : "Approved",
    ...
    public static IConfiguration BuildValidApiConfiguration()
    {
        ...
        {
            ["ConnectionStrings:ResourcesDb"] = "Host=localhost;Port=5432;Database=resources_tests;Username=postgres;Password=postgres",
