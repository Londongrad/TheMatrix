...
namespace Matrix.BuildingBlocks.Api.Tests.Authorization
{
    public sealed class JwtAuthenticationExtensionsTests
    {
        [Fact]
        public async Task
            AddJwtBearerAuthentication_WhenConfigurationIsValid_ConfiguresAuthenticationAndJwtBearerOptions()
        {
            ...
            ExternalJwtOptions jwtOptions = provider.GetRequiredService<IOptions<ExternalJwtOptions>>()
               .Value;
            ...
            Assert.Equal(
                ...
                actual: defaultScheme!.Name);
            Assert.Equal(
                expected: "https://issuer.test",
                actual: jwtOptions.Issuer);
            Assert.Equal(
                expected: "matrix-clients",
                actual: bearerOptions.TokenValidationParameters.ValidAudience);
        ...
        public void AddJwtBearerAuthentication_WhenIssuerIsMissing_ThrowsOptionsValidationExceptionOnResolve()
        {
            ...
            OptionsValidationException exception = Assert.Throws<OptionsValidationException>(() => provider
               .GetRequiredService<IOptions<ExternalJwtOptions>>()
               .Value);

            Assert.Contains(
                expectedSubstring: "Issuer is required",
        ...
        [Fact]
        public void
            AddInternalJwtAuthentication_WhenConfigured_RegistersCompositeSchemeAndResolvesForwardingByTokenKind()
        {
            ServiceCollection services = new();
            ...
            PolicySchemeOptions compositeOptions =
                provider.GetRequiredService<IOptionsMonitor<PolicySchemeOptions>>()
                   .Get(JwtAuthenticationExtensions.InternalCompositeJwtScheme);
            JwtBearerOptions userContextOptions =
                provider.GetRequiredService<IOptionsMonitor<JwtBearerOptions>>()
                   .Get(JwtAuthenticationExtensions.InternalUserContextJwtScheme);
            JwtBearerOptions serviceOptions =
                provider.GetRequiredService<IOptionsMonitor<JwtBearerOptions>>()
                   .Get(JwtAuthenticationExtensions.InternalServiceJwtScheme);

            DefaultHttpContext serviceContext = new();
            serviceContext.Request.Headers.Authorization =
                $"Bearer {CreateUnsignedJwt(new Claim(type: JwtClaimNames.InternalTokenKind, value: InternalJwtTokenKinds.Service))}";

            DefaultHttpContext userContext = new();
            userContext.Request.Headers.Authorization =
                $"Bearer {CreateUnsignedJwt(new Claim(type: JwtClaimNames.InternalTokenKind, value: InternalJwtTokenKinds.UserContext))}";
            ...
            Assert.Equal(
                expected: JwtAuthenticationExtensions.InternalServiceJwtScheme,
                actual: compositeOptions.ForwardDefaultSelector!(serviceContext));
            Assert.Equal(
                expected: JwtAuthenticationExtensions.InternalUserContextJwtScheme,
                actual: compositeOptions.ForwardDefaultSelector!(userContext));
            Assert.Equal(
                expected: JwtAuthenticationExtensions.InternalUserContextJwtScheme,
                actual: compositeOptions.ForwardDefaultSelector!(malformedContext));
