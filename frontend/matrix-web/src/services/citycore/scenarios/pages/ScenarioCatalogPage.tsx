...
function canComposeScenario(kind: string, can: (permission: string) => boolean): boolean {
    switch (kind) {
        case "ClassicCity":
            return can(PermissionKeys.CityCoreClassicCityCreate);
        default:
            return false;
    }
}
...
function canOpenScenario(kind: string, can: (permission: string) => boolean): boolean {
    switch (kind) {
        case "ClassicCity":
            return can(PermissionKeys.CityCoreClassicCityRead);
        default:
            return false;
    }
}
...
export default function ScenarioCatalogPage() {
    ...
    const primaryScenario = cityCoreScenarioRegistry[0];
    ...
    return (
        <section className="scenario-catalog">
            ...
            <div className="scenario-catalog__grid">
                {cityCoreScenarioRegistry.map((scenario, index) => {
