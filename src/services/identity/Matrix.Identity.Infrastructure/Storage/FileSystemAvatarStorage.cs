...
namespace Matrix.Identity.Infrastructure.Storage
{
    ...
    public sealed class FileSystemAvatarStorage(IHostEnvironment env) : IAvatarStorage
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            string wwwrootPath = Path.Combine(path1: env.ContentRootPath, path2: "wwwroot");
            string avatarsRoot = Path.Combine(path1: wwwrootPath, path2: "avatars");

            Directory.CreateDirectory(avatarsRoot);

            // имя файла: новый Guid + исходное расширение
            string finalFileName = $"{Guid.NewGuid()}{ext}";
            string physicalPath = Path.Combine(path1: avatarsRoot, path2: finalFileName);
            ...
            await using (var fs = new FileStream(
                ...
                await content.CopyToAsync(destination: fs, cancellationToken: cancellationToken);

            // относительный путь, который пойдёт в AvatarUrl и в <img src="...">
            // /avatars/xxx.png
            string relativePath = Path.Combine(path1: "avatars", path2: finalFileName)
        ...
        public Task DeleteAsync(string path, CancellationToken cancellationToken = default)
        {
            ...
            string wwwrootPath = Path.Combine(path1: env.ContentRootPath, path2: "wwwroot");
            string physicalPath = Path.Combine(path1: wwwrootPath, path2: relative);
