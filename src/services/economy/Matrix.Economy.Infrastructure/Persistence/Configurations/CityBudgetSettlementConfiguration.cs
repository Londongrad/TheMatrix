...
namespace Matrix.Economy.Infrastructure.Persistence.Configurations
{
    public sealed class CityBudgetSettlementConfiguration : IEntityTypeConfiguration<CityBudgetSettlement>
    {
        public void Configure(EntityTypeBuilder<CityBudgetSettlement> builder)
        {
            ...
            builder.Property(x => x.TickId).HasColumnName("tick_id");
            builder.Property(x => x.CurrentDate).HasColumnName("current_date");
            builder.Property(x => x.SettledDays).HasColumnName("settled_days");
            builder.Property(x => x.HouseholdCount).HasColumnName("household_count");
            builder.Property(x => x.ResidentCount).HasColumnName("resident_count");
            ...
            builder.HasIndex(x => new { x.CityId, x.TickId }).IsUnique();
