...
namespace Matrix.Population.Domain.Services
{
    public sealed class PopulationGenerator
    {
            ...
            DateOnly currentDate)
        {
            ...
            PersonName name = CreateRandomName(random: random, sex: sex);
            ...
            HealthLevel health = CreateRandomHealth(random: random, ageYears: ageYears);

            BodyWeight weight = CreateRandomWeight(random: random, sex: sex, ageYears: ageYears);

            var personality = Personality.CreateRandom(random);

            EmploymentStatus employmentStatus = CreateRandomEmploymentStatus(random: random, ageGroup: ageGroup);
            ...
            HappinessLevel happiness =
                CreateInitialHappiness(random: random, ageGroup: ageGroup, employmentStatus: employmentStatus);

            EducationLevel educationLevel = CreateRandomEducationLevel(r: random, ageYears: ageYears);

            MaritalStatus maritalStatus =
                CreateRandomMaritalStatus(random: random, ageGroup: ageGroup, ageYears: ageYears);
        ...
        private static Sex CreateRandomSex(Random random) =>
            random.Next(minValue: 0, maxValue: 2) == 0 ? Sex.Male : Sex.Female;
        ...
        private static PersonName CreateRandomName(Random random, Sex sex)
        {
            ...
            return new PersonName(firstName: firstName,
        ...
        private static BodyWeight CreateRandomWeight(Random random, Sex sex, int ageYears)
        {
            decimal kg = ageYears switch
            {
                ...
                < 1 => random.Next(minValue: 3, maxValue: 11), // 3–10
                < 3 => random.Next(minValue: 8, maxValue: 16), // 8–15

                // дошкольники
                < 7 => random.Next(minValue: 12, maxValue: 26), // 12–25

                // дети
                < 13 => random.Next(minValue: 20, maxValue: 46), // 20–45

                // подростки (уже заметна разница по полу)
                < 18 => sex == Sex.Male
                    ? random.Next(minValue: 40, maxValue: 86) // 40–85
                    : random.Next(minValue: 38, maxValue: 76), // 38–75

                // взрослые
                < 66 => sex == Sex.Male
                    ? random.Next(minValue: 60, maxValue: 111) // 60–110
                    : random.Next(minValue: 45, maxValue: 96), // 45–95

                // пожилые
                _ => sex == Sex.Male
                    ? random.Next(minValue: 55, maxValue: 96) // 55–95
                    : random.Next(minValue: 45, maxValue: 86) // 45–85
            };
        ...
        private static HealthLevel CreateRandomHealth(Random random, int ageYears)
        {
            ...
            {
                < 7 => random.Next(minValue: 70, maxValue: 101), // маленькие дети
                < 18 => random.Next(minValue: 60, maxValue: 101), // подростки
                < 40 => random.Next(minValue: 50, maxValue: 96), // взрослые
                < 66 => random.Next(minValue: 45, maxValue: 91), // 40–65
                < 80 => random.Next(minValue: 35, maxValue: 86), // пожилые
                _ => random.Next(minValue: 30, maxValue: 81) // 80+
            };

            // Доп. спад после 40 (нарастающий)
            if (ageYears > 40)
            {
                int extraPenalty = Math.Min(val1: 20, val2: (ageYears - 40) / 2); // 0..20
                value -= random.Next(minValue: 0, maxValue: 6) + extraPenalty; // -0..5 - extra
            }

            value = Math.Clamp(value: value, min: 0, max: 100);
        ...
        private static int CreateRandomAgeYears(Random random)
        {
            ...
            if (roll < 0.2) // дети
                return random.Next(minValue: 0, maxValue: 18); // [0..17]

            if (roll < 0.8) // взрослые
                return random.Next(minValue: 18, maxValue: 66); // [18..65]

            // пенсионеры
            return random.Next(minValue: 66, maxValue: 121); // [66..120]
        }
            ...
            EmploymentStatus employmentStatus)
        {
            ...
            int value = random.Next(minValue: 40, maxValue: 81); // [40..80]

            if (employmentStatus == EmploymentStatus.Employed && ageGroup == AgeGroup.Adult)
                value += random.Next(minValue: 0, maxValue: 11); // +0..10
            else if (employmentStatus == EmploymentStatus.Unemployed && ageGroup == AgeGroup.Adult)
                value -= random.Next(minValue: 5, maxValue: 16); // -5..15
            else if
                (employmentStatus == EmploymentStatus.Retired)
                value += random.Next(minValue: 0, maxValue: 6); // пенсионеры чуть довольнее
        ...
        private static Job? CreateRandomJob(Random random)
        {
            ...
            return new Job(workplaceId: workplaceId, title: title);
        }
