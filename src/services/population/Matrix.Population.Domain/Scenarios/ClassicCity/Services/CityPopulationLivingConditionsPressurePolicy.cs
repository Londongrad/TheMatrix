    ...
    public sealed class CityPopulationLivingConditionsPressurePolicy
    {
            ...
            CityPopulationEssentialsContext essentials)
        {
            ...
            double ageVulnerability = person.GetAgeGroup(currentDate) is AgeGroup.Child or AgeGroup.Senior
                ? 1.20d
                : 1.00d;
            double housingVulnerability = housingStatus == HousingStatus.Homeless ? 1.25d : 1.00d;
            double activityVulnerability = person.Employment.Status is EmploymentStatus.Employed or EmploymentStatus.Student
                ? 1.15d
                : 0.75d;
            double illnessVulnerability = person.HasActiveIllness ? 1.20d : 1.00d;
            ...
            return new CityPopulationLivingConditionsPressureEffect(
                HealthDelta: -ClampLoss(healthLoss, 10),
                EnergyDelta: -ClampLoss(energyLoss, 18),
                StressDelta: ClampLoss(stressGain, 18),
                HappinessDelta: -ClampLoss(happinessLoss, 14));
        }
        ...
        private static int ClampLoss(double value, int maxMagnitude)
        {
            int rounded = (int)Math.Round(value, MidpointRounding.AwayFromZero);
            return Math.Clamp(rounded, 0, maxMagnitude);
        }
    }
}
