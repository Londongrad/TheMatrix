...
namespace Matrix.Population.Application.Mapping
{
    public static class PersonMappingExtensions
    {
        ...
        public static PersonDto ToDto(this Person person, DateOnly currentDate)
        {
            ...
            int age = person.GetAge(currentDate).Years;
            string ageGroup = person.GetAgeGroup(currentDate).ToString();

            string birthDateStr = person.Life.BirthDate
                .ToString(format: "dd MMMM yyyy", provider: CultureInfo.InvariantCulture);
