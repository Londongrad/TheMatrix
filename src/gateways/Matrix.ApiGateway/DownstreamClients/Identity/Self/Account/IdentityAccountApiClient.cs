...
namespace Matrix.ApiGateway.DownstreamClients.Identity.Self.Account
{
    public sealed class IdentityAccountApiClient(HttpClient httpClient) : IIdentityAccountClient
    {
        ...
        public async Task<CursorPagedResult<SecurityActivityItemResponse>> GetSecurityActivityFeedAsync(
            string? cursor,
            int pageSize,
            CancellationToken cancellationToken)
        {
            ...
            return await response.ReadJsonOrThrowDownstreamAsync<CursorPagedResult<SecurityActivityItemResponse>>(
                serviceName: ServiceName,
                cancellationToken: cancellationToken,
                requestUrl: url);
        }
